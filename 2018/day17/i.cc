...
class ac_machine
{
    ...
    void init_block(void)
    {
        ...
        if(nl <= threshold)
        {
            ...
            for(i = 1;i <= n;i++)
            {
                int Li = lid(l[i]), L = xid(l[i]);
                max_lid[Li] = max(max_lid[Li], L);
                trs[Li/block_sz].push_back(train(i, L, xid(r[i])));
                assert(Li/block_sz < nblock);
            }
        }
    ...
    void upd(int L, int R, int delta, ll ratio, int o = 1, int l = 1, int r = nx)
    {
        ...
        if(L <= l && r <= R)
        {
            ...
            int mid = (l+r)>>1;
            pushdown(o);
            upd(L, R, delta, ratio, o<<1, l, mid);
            upd(L, R, delta, ratio, o<<1|1, mid+1, r);
        }
    }
    ...
    void disolve(int attack_id, int attack_pos)
    {
        ...
        if(block_id < nblock)
        {
            auto rightmost = lower_bound(trs[block_id].begin(), trs[block_id].end(), train(0,0,attack_pos));
            for(auto it = rightmost;it != trs[block_id].end();)
            {
                if(it->lid <= attack_pos)
                {
                    ans[it->id] = attack_id;
                    upd(it->lid, it->rid, -1, qow(it->id, mod-2));
                    trs[block_id].erase(it);
                } else {
                    it++;
                }
            }
