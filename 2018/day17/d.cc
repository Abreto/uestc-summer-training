...
public:
    ...
    abio &read_int(int &x)
    {
        ...
        x = 0; for (;(((ch>='0')&&(ch<='9')));ch=getchar()) x = x * 10 + (ch - '0');
        if ( flag ) x*=(-1);
        return (*this);
    }
    abio &read_ll(long long int &x)
    {
        ...
        for (;(EOF!=ch)&&((ch<'0')||(ch>'9'));ch=getchar()) if ('-'==ch) flag = 1;
        if ( EOF == ch ) return (this->reach_eof());
        x = 0ll; for (;(((ch>='0')&&(ch<='9')));ch=getchar()) x = x * 10ll + (ch - '0');
        if ( flag ) x*=(-1ll);
        return (*this);
    }
...
class ac_machine
{
    ...
    void proc(int casid)
    {
        ...
        while(k--)
        {
            int ai, bi; ll ci;
            io>>ai>>bi>>ci;
            max_c[bi] = max(max_c[bi], ci);
        }
        ll ans = 0;
        for(k = 1;k <= m;k++)
            ans += max_c[k];
        io<<"Case #"<<casid<<": "<<ans<<io.endl;
    }
public:
    int wa()
    {
