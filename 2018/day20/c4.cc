...
struct bits_t
{
    ...
    bool operator<(const bits_t &b) const
    {
        ...
        assert((1<<n) == bits.size());
        assert((1<<(b.n)) == b.bits.size());
        for(int i = 0;i < (1<<n);i++)
            if(bits[i] != b.bits[i])
...
#define MAXVERT 2500000
int nvertices;
map< bits_t, int > ids;
string tobits[MAXVERT];
void str_display( int id )
{
    for (int i = 0;i < tobits[id].size(); i++ )
        fprintf(stderr, "%c", tobits[id][i] + '0');
    fprintf(stderr, "\n");
}
int getId( bits_t &x )
{
    if( ids.find(x) == ids.end() )
    {
        ...
        // fprintf(stderr, "In GetID: ");
        // str_display( nvertices );
    }
    return ids[x];
}
int tree[MAXVERT];
    ...
    int wa(void)
    {
        ...
        for(int v = 1;v <= nvertices;v++)
        {
            ...
            for(i = 0;i < len;i += 2)
            {
                for(j = 0;j < 3;j++)
                {
                    int l = tobits[v][i], r = tobits[v][i+1];
                    assert(l < 2); assert(r < 2);
                    newbits[j].bits[i >> 1] = op[j][ l ][ r ];
                }
            }
            for(j = 0;j < 3;j++)
            {
