...
public:
    int wa(void)
    {
        ...
        for(i = 0;i < n;i++)
        {
            ...
            left[i][0] = 0; last[ gird[i][0] ] = 0;
            for(j = 1;j < m;j++)
            {
                left[i][j] = max( left[i][j-1], last[ gird[i][j] ] + 1 );
                last[ gird[i][j] ] = j;
            }
        }
        for(j = 0;j < m;j++)
        {
            memset(last, -1, sizeof(last));
            up[0][j] = 0; last[ gird[0][j] ] = 0;
            for(i = 1;i < n;i++)
            {
                up[i][j] = max( up[i-1][j], last[ gird[i][j] ] + 1 );
                last[ gird[i][j] ] = i;
            }
        }
        for(i = 0;i < n;i++)
        {
            for(j = 0;j < m;j++)
            {
                fprintf(stderr, "(%d,%d)", left[i][j], up[i][j]);
            }
            fputs("\n", stderr);
        }
        ll ans = 0;
        for(i = 0;i < n;i++)
        {
            for(j = 0;j < m;j++)
            {
                memset(top, 0x7f, sizeof(top));
                for(k = j;k >= left[i][j];k--)
                {
                    top[k] = min(top[k+1], up[i][k]);
                }
                int l = left[i][j];
                for(k = i;k >= up[i][j];k--)
                {
