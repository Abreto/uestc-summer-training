...
public:
    int wa(void)
    {
        ...
        tree[bits] = 1; next_floor[(n & 1) ^ 1].insert(getId(bits));
        for(w = n;w > 0;w--)
        {
            ...
            for (auto v : next_floor[(w & 1) ^ 1])
            {
                ...
                for(i = 0;i < (1<<w);i += 2)
                {
                    for(j = 0;j < 3;j++)
                    {
                        newbits[j].bits[i >> 1] = op[j][ v.bits[i] ][ v.bits[i+1] ];
                    }
                }
                for(j = 0;j < 3;j++)
                {
                    tree[getId(newbits[j])] += base;
                    if( tree.find( newbits[j] ) == tree.end() )
                    {
                        tree[newbits[j]] = base;
                    } else {
                        tree[newbits[j]] += base;
                    }
                    next_floor[w & 1].insert(get(newbits[j]);
                }
            }
        }
        bits.setn(0); bits.bits[0] = 1;
        io << tree[bits] << io.endl;
        // if( tree.finds( bits ) == tree.end() ) tree[bits] = 0;
        return 0;
    }
};
const int ac_machine::op[3][2][2];
