...
class ac_machine
{
    ...
    static const int koffset = 256;
    static const int s = 300, sp = 301, tp = 302, t = 303;
    ...
    bool check(int goal)
    {
        ...
        for (int e = mcmf::node[koffset + T].head; e != -1; e = mcmf::edge[e].next)
            if (mcmf::edge[e].to == t)
    ...
    {
        mcmf::init(t + 1);
        mcmf::add_edge(s, sp, N, 0);
        for (int i = 1; i <= N; i++)
        {
            mcmf::add_edge(sp, i, 1, 0);
            for (int j = 1; j <= K + 1; j++)
            {
                mcmf::add_edge(i, koffset + j, 1, C[i][j]);
            }
        }
        for (int j = 1; j <= K; j++)
        {
            if (j == T) mcmf::add_edge(koffset + j, t, goal, 0);
            else mcmf::add_edge(koffset + j, tp, goal - 1, 0);
        }
        mcmf::add_edge(koffset + K + 1, tp, inflow, 0);
        ...
        if (candidate < ans)
        {
            ...
            for (int i = 1; i <= N; i++)
            {
                ...
                for (int e = mcmf::node[i].head; e != -1; e = mcmf::edge[e].next)
                    if ((mcmf::edge[e].to > koffset) && (mcmf::edge[e].to <= koffset + K + 1) && mcmf::edge[e].flow == 1)
                    {
                        ch = mcmf::edge[e].to - koffset;
                        break;
                    }
                chk.pb(ch);
            }
