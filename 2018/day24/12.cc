...
namespace repo
{
    ...
    {
        ll cost, sz;
        item(ll c = 0, ll s = 0) : cost(c), sz(s) { }
...
class ac_machine
{
    static const int maxk = 50500;
    int k;
    ll c[maxk], d[maxk], m[maxk], p[maxk];
    ll e[maxk], R[maxk], E[maxk];
    ll ans; bool fail;
    int ac(void)
    {
        ll Esum = 0;
        for (int i = 1;i < k;i++) c[i] = min(c[i], c[i-1] + R[i]);
        repo::clear();
        e[k] = 0;
        for (int i = 0;i < k;i++)
        {
            if (p[i])
            {
                repo::push(repo::item(c[i] + m[i] - Esum, p[i]));
            }
            while (d[i] > 0)
            {
                if (0 == repo::restore) { return true; }
                auto produce = repo::pop_front();
                int satisify = min(produce.sz, d[i]);
                produce.sz -= satisify;
                d[i] -= satisify;
                ans += satisify * (produce.cost + Esum);
                if (produce.sz) repo::push(produce);
            }
            ll rem = max(repo::restore - e[i + 1], 0ll);
            while (rem > 0)
            {
                auto abondon = repo::pop_back();
                int drop = min(abondon.sz, rem);
                rem -= drop;
                abondon.sz -= drop;
                if (abondon.sz) repo::push(abondon);
            }
            Esum += E[i + 1];
        }
        return false;
