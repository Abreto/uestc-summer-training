...
bool if_calced(int cakei, int a, int b, int layers)
{
  char cs[64];
  string s;
  sprintf(cs, "(%d,%d,%d,%d)", cakei, a, b, layers);
  s = cs;
  return calced[s];
}

void make_calced(int cakei, int a, int b, int layers)
{
  char cs[64];
  string s;
  sprintf(cs, "(%d,%d,%d,%d)", cakei, a, b, layers);
...
void dfs(int cakei, int a, int b, int layers)
{
  ...
  if( cakes[cakei].b <= b )
  {
    ...
    if( definitely_lose <= possibly_add )
